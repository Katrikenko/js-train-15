...
function roundObjectValues(obj) {
  ...
  const getEnteries = Object.entries(obj);

  const map = getEnteries.map(([key, value]) => {
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== "number" || typeof height !== "number") {
    console.log("Error: radius or height are not numbers");
    return null;
  }

  let cylinderVol = Math.PI * Math.pow(radius, 2) * height;
  let volume = Math.ceil(cylinderVol);
  return volume;
}
...
function getFractionalPart(num) {
  ...
  let getFrac = num - truncNum;

  let result = Math.fround(getFrac);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let convertTotalPrice = Math.fround(totalPrice);
  let convertMaxPrice = Math.fround(maxPrice);

  let verifyPrice = convertTotalPrice > convertMaxPrice;
